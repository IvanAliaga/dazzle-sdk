...
// Result types (AnomalyDecision, RiskPrediction, CheckpointResult, ReportScore,
// GroundTruth, SynthesisScore, SynthesisResult, ExperimentResults) are declared
// at the top level in ExperimentPipelineIoT.swift (main baseline). Both the
// Paper 2 and Valkey 8 variants share the same record types, so re-declaring
// them here would just duplicate identical layouts and break callers like the
// shared ResultsExporter.
//
// CheckpointResult in main declares an extra `var contextBlock: String = ""`
// field with a default value; Valkey 8 code doesn't populate it, which is fine —
// the default keeps the memberwise initialiser source-compatible.

// ─────────────────────────────────────────────────────────────────────────────

@MainActor
final class ExperimentPipelineIoTValkey8: ObservableObject {
    ...
    private func createBackend(name: String) -> StorageBackend {
        ...
        case "dazzle-vector":   return DazzleVectorIoTValkey8Manager()
