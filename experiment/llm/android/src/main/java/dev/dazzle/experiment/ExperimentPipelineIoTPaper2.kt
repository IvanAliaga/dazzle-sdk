...
/**
 * ExperimentPipelineIoTPaper2 v2 — Three-Task Sequential Monitoring Agent
 *
 ...
 */
class ExperimentPipelineIoTPaper2(
    ...
    data class CheckpointResult(
        ...
        // Task 2 — LLM prediction (augmented only; stateless has no history)
        val prediction: RiskPrediction?,
        // Task 2 — rule-based prediction (model-agnostic companion to LLM)
        val rulePrediction: FaultRiskAssessment? = null,
        val rulePredictionCorrect: Boolean = false,
    ...
    fun run(): ExperimentResults {
        ...
        val storage: StorageBackend = when (backendName.lowercase()) {
            ...
            "dazzle-vector" -> {
                ensureServerRunning(modules = setOf(DazzleModule.VectorSearch, DazzleModule.TFI))
                DazzleVectorIoTValkey9Paper2Manager()
            }
        ...
        // Prefer v3 (with NAMUR NE43 status codes); DatasetLoader falls
        // back to v2 transparently if the APK doesn't ship v3 yet.
        val dataset = DatasetLoader.load(context, "dataset_iot_valkey9.json")
        ...
        val rulePredictions     = mutableListOf<FaultRiskAssessment?>()
        val ruleNextHasFault    = mutableListOf<Boolean>()
        var lastIngested        = -1
        ...
        for (cpIdx in 0 until numCPs) {
            ...
            // Online-learning feedback for the PREVIOUS CP's rule prediction.
            // At CP T we now know whether CP T's window was a fault, which is
            // the ground truth for the prediction emitted at CP T-1. Pass it
            // to TFI so its Bayesian posteriors converge over the session.
            if (cpIdx > 0) {
                val currentCpHasFault = dataset.windowHasAnomaly(cpIdx)
                storage.observeActualFault(currentCpHasFault)
            }
            ...
            // buildPredictionContext is self-contained: current brief + fault
            // history + precursor KNN. We intentionally do NOT concatenate the
            // full detection context here — that context anchors the LLM on
            // the present state ("currently normal") and drowns predictive
            // signals. Separation of concerns: detection vs prediction have
            // different optimal contexts.
            val nextHasFault   = if (cpIdx + 1 < numCPs) dataset.windowHasAnomaly(cpIdx + 1) else false
            val predContext    = storage.buildPredictionContext(cpReading.minute)
            ...
            // Rule-based deterministic prediction (model-agnostic companion).
            // Runs alongside the LLM call so we can compare both in the same
            // experiment and report hybrid / ablation numbers in the paper.
            val ruleAssessment = storage.computeRuleBasedRisk(cpReading.minute)
            rulePredictions.add(ruleAssessment)
            ruleNextHasFault.add(nextHasFault)
        }
        ...
        val checkpoints = (0 until numCPs).map { i ->
            val ruleAssess = rulePredictions.getOrNull(i)
            val ruleCorrect = if (ruleAssess != null && i < ruleNextHasFault.size)
                ruleAssess.predicted == ruleNextHasFault[i]
            else false
            CheckpointResult(
                cpIndex               = i,
                ...
                rulePrediction        = ruleAssess,
                rulePredictionCorrect = ruleCorrect,
                ...
                contextBlock          = contextBlocks.getOrElse(i) { "" },
            )
        }
    ...
    private fun printSummary(r: ExperimentResults) {
        ...
        // Task 2 — Rule-based prediction (model-agnostic companion)
        val ruleEntriesSum = r.checkpoints.filter { it.rulePrediction != null && it.prediction != null }
        val ruleAccSum = if (ruleEntriesSum.isEmpty()) 0.0
                         else ruleEntriesSum.count { it.rulePredictionCorrect }.toDouble() / ruleEntriesSum.size
        val ruleFaultCountSum = ruleEntriesSum.count { it.prediction!!.nextWindowHasFault }
        val ruleRecSum = if (ruleFaultCountSum == 0) 0.0
                         else ruleEntriesSum.count { it.prediction!!.nextWindowHasFault && it.rulePrediction!!.predicted }.toDouble() / ruleFaultCountSum
        val hybridRecSum = if (ruleFaultCountSum == 0) 0.0
                           else ruleEntriesSum.count {
                               val llmPos = (it.prediction!!.riskPct >= 50)
                               it.prediction.nextWindowHasFault && (llmPos || it.rulePrediction!!.predicted)
                           }.toDouble() / ruleFaultCountSum
        ...
        val summary = """
            ...
            Task 2 — Risk Prediction (augmented only):
              LLM       : accuracy=${pct(predAcc)}  fault-recall=${pct(predRecall)}
              Rule-based: accuracy=${pct(ruleAccSum)}  fault-recall=${pct(ruleRecSum)}
              Hybrid OR : fault-recall=${pct(hybridRecSum)}
    ...
    private fun saveResults(results: ExperimentResults) {
        ...
        // Three-level fallback for Android storage restrictions:
        //   1. Public Documents (/sdcard/Documents) — preferred for adb pull,
        //      works on Android ≤9 and on devices that grant legacy storage.
        //   2. App-scoped external (/sdcard/Android/data/<pkg>/files/Documents)
        //      — no permission needed on Android 10+, still adb-accessible.
        //   3. Internal filesDir — always writable but requires run-as to pull.
        val dir = try {
            val docs = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS)
            docs.mkdirs()
            if (docs.canWrite()) docs
            else context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS)?.also { it.mkdirs() }
                ?: context.filesDir
        } catch (_: Exception) {
            context.getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS)?.also { it.mkdirs() }
                ?: context.filesDir
        }
        ...
        try {
            ...
            // Last-resort fallback: internal filesDir is always writable.
            val fallback = File(context.filesDir, fileName)
            fallback.writeText(json)
            onProgress(-1, -1, "Results saved (fallback): ${fallback.absolutePath} — primary failed: ${e.message}")
        }
    }
    ...
    private fun serialise(r: ExperimentResults): Map<String, Any> {
        ...
        out["checkpoints"] = r.checkpoints.map { cp ->
            mapOf(
                ...
                "rule_prediction" to cp.rulePrediction?.let { a -> mapOf(
                    "probability"             to a.probability,
                    "predicted"               to a.predicted,
                    "prediction_correct"      to cp.rulePredictionCorrect,
                    "base_rate"               to a.baseRate,
                    "cluster_density"         to a.clusterDensity,
                    "interval_ratio"          to a.intervalRatio,
                    "precursor_match_pct"     to a.precursorMatchPct,
                    "current_physical_state"  to a.currentPhysicalState,
                    "fired_signals"           to a.firedSignals,
                )},
                ...
                "context_block"      to cp.contextBlock,
            )
        }
        ...
        // Rule-based prediction metrics — evaluated identically to LLM metrics
        // so paper can compare directly and report hybrid (LLM OR rule) numbers.
        // nextWindowHasFault is pulled from LLM prediction (they share ground truth).
        val ruleEntries = r.checkpoints.filter { it.rulePrediction != null && it.prediction != null }
        val ruleAcc = if (ruleEntries.isEmpty()) 0.0
                      else ruleEntries.count { it.rulePredictionCorrect }.toDouble() / ruleEntries.size
        val ruleFaultCount = ruleEntries.count { it.prediction!!.nextWindowHasFault }
        val ruleRec = if (ruleFaultCount == 0) 0.0
                      else ruleEntries.count { it.prediction!!.nextWindowHasFault && it.rulePrediction!!.predicted }.toDouble() / ruleFaultCount
        val ruleFpr = run {
            val normals = ruleEntries.filter { !it.prediction!!.nextWindowHasFault }
            if (normals.isEmpty()) 0.0
            else normals.count { it.rulePrediction!!.predicted }.toDouble() / normals.size
        }
        // Hybrid: LLM OR rule (union of positive predictions)
        val hybridRec = if (ruleFaultCount == 0) 0.0
                        else ruleEntries.count {
                            val llmPos = (it.prediction!!.riskPct >= 50)
                            val rulePos = it.rulePrediction!!.predicted
                            it.prediction.nextWindowHasFault && (llmPos || rulePos)
                        }.toDouble() / ruleFaultCount
        val hybridFpr = run {
            val normals = ruleEntries.filter { !it.prediction!!.nextWindowHasFault }
            if (normals.isEmpty()) 0.0
            else normals.count {
        ...
        out["metrics"] = mapOf(
            ...
            // Task 2 — Rule-based (LLM-agnostic)
            "prediction_accuracy_rule"       to ruleAcc,
            "prediction_fault_recall_rule"   to ruleRec,
            "prediction_fpr_rule"            to ruleFpr,
            // Task 2 — Hybrid (LLM OR rule)
            "prediction_fault_recall_hybrid" to hybridRec,
            "prediction_fpr_hybrid"          to hybridFpr,
            // Task 3
