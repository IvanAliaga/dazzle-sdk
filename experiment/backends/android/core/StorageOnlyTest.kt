...
object StorageOnlyTest {
    ...
    fun run(context: Context, backendName: String) {
        ...
        // tcpEnabled is on so the Valkey baseline can connect via real TCP
        // loopback (the path a Lettuce/Jedis-style mobile client takes today);
        // Dazzle backends still take the in-process pipe route — both
        // transports coexist on the same server.
        ...
        // Full vector benchmark: dazzle-vector (HNSW, ef sweep) vs SQLite
        // brute-force. Writes one consolidated JSON per run.
        if (backendName.lowercase() == "vector-bench") {
            VectorBenchmark.run(context)
            return
        }

        // Hardcore sweep — BGE-large / OpenAI-3-large scales.
        // Extends the paper comparison with dim=768 and 1024 across
        // N={10 k, 100 k}. Exercises the quantised-scan limit of
        // sqlite-vector-ai and the HNSW-SQ8 ingest/query balance on
        // mid-range Android at realistic corpus sizes.
        if (backendName.lowercase() == "vector-bench-hardcore") {
            VectorBenchmark.run(context, listOf(
                VectorBenchmark.Config(dim =  768, nDocs =  10_000),
                VectorBenchmark.Config(dim = 1024, nDocs =  10_000),
                VectorBenchmark.Config(dim =  768, nDocs = 100_000,
                                       nQueries = 50,
                                       efRuntimes = intArrayOf(10, 50, 200)),
                VectorBenchmark.Config(dim = 1024, nDocs = 100_000,
                                       nQueries = 50,
                                       efRuntimes = intArrayOf(10, 50, 200)),
            ))
            return
        ...
        // Concurrent-query throughput bench: fans dim=384 N=10k queries across
        // 1/2/4/8 worker threads, records QPS + per-query latency under load.
        if (backendName.lowercase() == "vector-throughput") {
            VectorThroughputBench.run(context)
            return
        }

        // Realistic-dim sweep for dazzle only — no sqlite-vec / ObjectBox /
        // sqlite-brute at these dims (they'd be either broken or pointlessly
        // slow with predictable results). Validates that the R0→R12 wins at
        // dim=384 generalise to 768 (BGE-base, E5) and 1536 (OpenAI
        // text-embedding-3-*, ada-002). Ground truth comes from a local
        // brute-force on a small query set inside the bench itself.
        if (backendName.lowercase() == "vector-dim-sweep") {
            VectorDimSweep.run(context)
            return
        }

        // R13 validation: measure query p50/p95 while a writer appends new
        // vectors concurrently. Quiet vs live-append regime comparison.
        if (backendName.lowercase() == "vector-live-ingest") {
            VectorLiveIngestBench.run(context)
            return
        }

        // E1 embedder bench: on-device llama.cpp + GGUF. Measures cold start,
        // per-doc latency across 3 length buckets (~50/100/200 tokens) and
        // sustained throughput. Needs the GGUF pushed to /sdcard/Download/.
        if (backendName.lowercase() == "embed-bench") {
            EmbedLatencyBench.run(context)
            return
        }

        // E2 end-to-end RAG retrieval recall. Pipeline: GGUF embedder → HNSW
        // (dazzle-vector) over the committed NQ mini-slice (2k passages, 200
        // queries, one gold passage per query). Reports recall@{1,5,10},
        // per-query embed + search latency, and index build time.
        if (backendName.lowercase() == "rag-bench") {
            RagRecallBench.run(context)
            return
        }

        // E3 end-to-end RAG narrative: small LLM + dazzle-vector RAG vs
        // large LLM with no retrieval. Both generate greedy on the NQ
        // slice; snapshot captures token-F1, prefill/decode latency
        // breakdown, and prompt/new-token counts per variant.
        if (backendName.lowercase() == "rag-e2e") {
            RagE2EBench.run(context)
            return
        }

        // Fase 1 paridad HNSW: lee el dataset determinista generado por
        // research/scripts/hnsw_parity.py (dim=384, N ∈ {1k,10k,100k},
        // 1k queries, numpy brute-force GT) desde /sdcard/Documents/
        // hnsw_parity/, corre dazzle-vector con M=16/efC=200 y sweep
        // ef ∈ {16..512}. El JSON es comparable bit-a-bit con la corrida
        // x86 para medir paridad del port hnswlib v0.8.0.
        if (backendName.lowercase() == "hnsw-parity") {
            HnswParityBench.run(context)
            return
        }

        // Smoke configuration of the same harness for fast CI runs
        // (one small dim, one small N). Useful before the long sweep.
        if (backendName.lowercase() == "vector-bench-smoke") {
            VectorBenchmark.run(context, listOf(
                VectorBenchmark.Config(dim = 16, nDocs = 500, nQueries = 50),
            ))
            return
        }

        // SQLite-family-only vector benchmark for reviewer mitigation:
        // sqlite / sqlite-vec / sqlite-vector-ai with default/optimized/
        // precompute variants. Faster than the full multi-engine sweep.
        if (backendName.lowercase() == "vector-bench-sqlite-family") {
            VectorSqliteFamilyBenchmark.run(context)
            return
        }

        // Mirror of the iOS `paper384_scale` preset. Single dim (384) ×
        // N ∈ {200, 1k, 5k, 20k}. This is the configuration used to
        // populate Table 4 (vector backends N=200 envelope) and the
        // Android↔iOS parity row for Dazzle-Vector. Keep this list in
        // lock-step with experiment/backends/ios/App/BackendsApp.swift
        // `paper384_scale` so the two devices report on the same grid.
        if (backendName.lowercase() == "vector-bench-paper384-scale" ||
            backendName.lowercase() == "vector-bench-paper384-scale-no-dazzle") {
            val skip = backendName.lowercase().endsWith("-no-dazzle")
            VectorBenchmark.run(context, listOf(
            ...
            ), forceSkipDazzle = skip)
            return
        }

        // Same benchmark as above, but with N sweep
        // (N in {500, 2k, 10k}) for Table 5.8 coverage.
        if (backendName.lowercase() == "vector-bench-sqlite-family-sweep") {
            VectorSqliteFamilyBenchmark.runSweep(context)
            return
        }

        // DazzleVectorIoTValkey9Manager unit tests — validates metadata persistence,
        // faultPct correctness, and MEMORY ASSESSMENT line.
        if (backendName.lowercase() == "dazzle-vector-test") {
            DazzleVectorIoTValkey9ManagerTest.run(context)
            return
        }
        ...
        // Pre-ingest snapshots. The PSS / RSS pair is one quiesce + two
        // back-to-back kernel reads (see MemoryProbe.Snapshot) so the
        // primary number agrees with the explicit pss/rss fields by
        // construction. The backend-attributable byte count is the
        // primary defensible metric at small N — process-wide PSS at
        // N=200 is dominated by GC noise.
        val ramBefore = MemoryProbe.snapshot()
        val backendBytesBefore = backend.backendSizeBytes()
        val backendBreakdownBefore = backend.backendSizeBreakdown()
        ...
        val backendBreakdownAfter = backend.backendSizeBreakdown()
        ...
        val result = linkedMapOf<String, Any?>(
            ...
            "backend_size_breakdown_before_bytes" to (backendBreakdownBefore ?: emptyMap()),
            "backend_size_breakdown_after_bytes"  to (backendBreakdownAfter  ?: emptyMap()),
            "backend_size_breakdown_delta_bytes" to run {
                val before = backendBreakdownBefore ?: emptyMap()
                val after = backendBreakdownAfter ?: emptyMap()
                after.mapNotNull { (k, v) ->
                    before[k]?.let { k to (v - it) }
                }.toMap()
            },
