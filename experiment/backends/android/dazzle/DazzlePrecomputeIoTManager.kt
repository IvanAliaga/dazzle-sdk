 ...
 */
class DazzlePrecomputeIoTManager : StorageBackend {
    ...
    private val ingestScript = dazzle.script("""
        ...
        --    Design principle: this block is OPERATIONAL STATE, not raw data.
        --    The window temps are already in the checkpoint prompt — repeating
        --    them here adds noise. What the model cannot derive from the prompt
        --    alone: session-wide stats, trend, and fault history.
        local lines = {}
        lines[#lines+1] = '[Sensor State]'
        if new_count and new_count > 0 then
            local avg = new_sum / new_count
            lines[#lines+1] = 'Session: ' .. new_count .. ' readings | ' ..
                'avg=' .. string.format('%.1f', avg) .. '~DEG~C | ' ..
                'range=' .. string.format('%.1f', cur_min) .. '~ARR~' ..
                string.format('%.1f', cur_max) .. '~DEG~C | ' ..
                'trend=' .. trend
            if anom_cnt > 0 then
                lines[#lines+1] = 'Fault events logged this session: ' .. anom_cnt
            else
                lines[#lines+1] = 'No fault events logged this session.'
            end
        end
        if anoms_csv ~= '' then
            lines[#lines+1] = 'Active fault window (last ' .. win_min .. ' min) — fault at minutes: [' .. anoms_csv .. ']'
        end
