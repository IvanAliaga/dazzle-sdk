 ...
 *
 * `sqlite-optimized` already materializes scalar aggregates via trigger.
 * This variant goes one step further and also stores a pre-rendered
 * context block (`ctx_block`) on ingest, so retrieval is a single-row read.
 */
class SqlitePrecomputeContextManager(context: Context) : StorageBackend {

    override val backendName: String = "SQLite-Precompute"

    private val db: SQLiteDatabase

    init {
        val helper = object : SQLiteOpenHelper(context, "sensor_experiment_precompute.db", null, 1) {
            override fun onCreate(db: SQLiteDatabase) {
                ...
                """)
                db.execSQL("CREATE INDEX idx_readings_minute_pre ON readings(minute)")
                ...
                db.execSQL("""
                    CREATE TRIGGER readings_after_insert_agg_pre
                ...
                """)
                db.execSQL("INSERT OR IGNORE INTO context_cache (id) VALUES (1)")
            ...
            override fun onUpgrade(db: SQLiteDatabase, old: Int, new: Int) {
                ...
                db.execSQL("DROP TABLE IF EXISTS context_cache")
                db.execSQL("DROP TABLE IF EXISTS decisions")
                db.execSQL("DROP TABLE IF EXISTS checkpoints")
                db.execSQL("DROP TRIGGER IF EXISTS readings_after_insert_agg_pre")
                onCreate(db)
            }
        }
    ...
    override fun flush() {
        ...
        db.execSQL("DELETE FROM context_cache")
    ...
    override fun ingest(reading: SensorReading) {
        ...
        refreshContextCache(reading.minute, 20)
    }

    override fun buildContextBlock(currentMinute: Int, windowMinutes: Int): String {
        db.rawQuery(
            "SELECT ctx_block FROM context_cache WHERE id = 1",
            null,
        ).use { c ->
            if (c.moveToFirst()) {
                val cached = c.getString(0)
                if (!cached.isNullOrEmpty()) return cached
            }
        }
        return computeContextBlockFor(currentMinute, windowMinutes)
    }
    ...
    private fun refreshContextCache(currentMinute: Int, windowMinutes: Int) {
        val ctx = computeContextBlockFor(currentMinute, windowMinutes)
        db.execSQL(
            """
            ...
            """.trimIndent(),
            arrayOf(currentMinute, ctx),
        )
    }

    private fun computeContextBlockFor(currentMinute: Int, windowMinutes: Int): String {
        val lines = mutableListOf<String>()
