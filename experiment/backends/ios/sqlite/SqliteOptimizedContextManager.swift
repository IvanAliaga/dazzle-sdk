...
/// SQLite backend with write-time materialized aggregates maintained by trigger.
///
/// This mirrors the Dazzle-Precompute access pattern (compute on write,
/// O(1)-style scalar read on retrieval) while staying inside plain SQLite.
final class SqliteOptimizedContextManager: StorageBackend {

    let backendName: String = "SQLite-Optimized"
    ...
    init() {
        ...
        let path = docs.appendingPathComponent("sensor_experiment_opt.db").path
        self.dbPath = path

        guard sqlite3_open(path, &db) == SQLITE_OK else {
            print("[SQLite-Optimized] Failed to open database at \(path)")
            return
        }
        ...
        """)
        exec("CREATE INDEX IF NOT EXISTS idx_readings_minute_opt ON readings(minute)")
        ...
        exec("""
            CREATE TRIGGER IF NOT EXISTS readings_after_insert_agg
    ...
    // MARK: - Context block (byte-identical to SQLite / Valkey outputs)
    ...
    private func exec(_ sql: String, bind: ((OpaquePointer) -> Void)? = nil) -> Bool {
        ...
        guard sqlite3_prepare_v2(db, sql, -1, &stmt, nil) == SQLITE_OK else {
            print("[SQLite-Optimized] prepare failed: \(String(cString: sqlite3_errmsg(db!)))")
            return false
        }
        ...
        if rc != SQLITE_DONE && rc != SQLITE_ROW {
            print("[SQLite-Optimized] step failed (\(rc)): \(String(cString: sqlite3_errmsg(db!)))")
            return false
        }
        return true
    }
