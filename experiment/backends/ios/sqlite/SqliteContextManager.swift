...
///
/// This is the apples-to-apples comparison against SensorContextManager.
/// It stores the exact same data and produces byte-identical context blocks
/// so the Gemma model sees an identical prompt regardless of backend.
///
/// Uses the C SQLite3 API directly (no wrapper library) since iOS ships
/// SQLite as a system framework.
final class SqliteContextManager: StorageBackend {

    let backendName: String = "SQLite"
    ...
    init() {
        ...
        let path = docs.appendingPathComponent("sensor_experiment.db").path
        self.dbPath = path

        guard sqlite3_open(path, &db) == SQLITE_OK else {
            print("[SQLite] Failed to open database at \(path)")
            return
        }
        ...
        """)
        exec("CREATE INDEX IF NOT EXISTS idx_readings_minute ON readings(minute)")
    ...
    func ingest(_ reading: SensorReading) {
        ...
        // Trim to last 200 readings (with buffer to avoid trimming every insert)
    ...
    // MARK: - Context block (byte-identical to Valkey output)
    ...
    private func readStats() -> RunningStats? {
        guard let count = getStat("count").map({ Int($0) }), count > 0 else { return nil }
        let sum     = getStat("temp_sum") ?? 0
        let minTemp = getStat("min_temp") ?? 0
        let maxTemp = getStat("max_temp") ?? 0
        let anomCnt = Int(getStat("anomaly_count") ?? 0)
        return RunningStats(
            count:        count,
    ...
    private func getStat(_ key: String) -> Double? {
        var result: Double?
        query("SELECT value FROM stats WHERE key = ?", bind: { stmt in
    ...
    private func upsertStat(_ key: String, _ value: Double, increment: Bool) {
        ...
        if existing == nil {
            exec("INSERT OR REPLACE INTO stats (key, value) VALUES (?, ?)", bind: { stmt in
                sqlite3_bind_text(stmt, 1, (key as NSString).utf8String, -1, nil)
                sqlite3_bind_double(stmt, 2, value)
            })
        } else {
            let newVal = increment ? existing! + value : value
            exec("UPDATE stats SET value = ? WHERE key = ?", bind: { stmt in
                sqlite3_bind_double(stmt, 1, newVal)
    ...
    private func exec(_ sql: String, bind: ((OpaquePointer) -> Void)? = nil) -> Bool {
        ...
        if rc != SQLITE_DONE && rc != SQLITE_ROW {
            print("[SQLite] step failed (\(rc)): \(String(cString: sqlite3_errmsg(db!)))")
            return false
        }
        return true
    }
    ...
    /// Sum of the main DB file plus its sidecars (-wal / -shm / -journal),
    /// matching the Android backend. WAL mode in particular keeps a
    /// separate write-ahead log that we must include for an honest
    /// per-backend on-disk byte count.
