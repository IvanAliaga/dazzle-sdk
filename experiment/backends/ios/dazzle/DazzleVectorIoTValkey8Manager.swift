...
final class DazzleVectorIoTValkey8Manager: StorageBackend {
    ...
    func ingest(_ reading: SensorReading) {
        ...
        if reading.anomalous {
            ...
            cmds.append(["PFADD", "sensor:anomaly_type_hll", anomalyType(tempC: reading.tempC)])
        }
    ...
    private func buildKnnContext(currentMinute: Int) -> String {
        ...
        let results = vIdx.search(query: query, k: Self.KNN_K,
        ...
        if !results.isEmpty {
            ...
            let patternDesc = faultPct >= 50 ? "were FAULT events." : "were normal."
            lines.append("Pattern signal: \(faultPct)% of the \(Self.KNN_K) most similar past readings \(patternDesc)")
    ...
    func buildPredictionContext(currentMinute: Int) -> String {
        ...
        let query   = encodeReading(tempC: latestTemp, minute: Double(currentMinute), anomalous: isAnom)
        let results = precursorIdx.search(query: query, k: Self.PRECURSOR_K,
    ...
    func buildSynthesisContext() -> String {
        ...
        if uniqueAnomalies > 0 {
            parts.append("[Anomaly Profile — HyperLogLog estimates]\n" +
