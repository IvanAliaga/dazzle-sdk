...
///
/// Wrapped in `extension Valkey8` so the enum coexists with the main baseline
/// (storage/ios/StorageOnlyTest.swift) and the Paper 2 variant.
extension Valkey8 {
...
enum StorageOnlyTest {
    ...
    static func run(backendName: String) {
        if backendName.lowercased() == "dazzle-vector" {
            ...
            print("[StorageTest] dazzle-vector storage test — use DazzleVectorIoTValkey8Manager via LLM experiment")
            return
        }
