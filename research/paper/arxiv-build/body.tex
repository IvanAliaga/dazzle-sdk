...
\subsubsection{5.9.5 Cross-Platform Extension to Three Android
SoCs}\label{cross-platform-extension-to-three-android-socs}

To check that the §5.9 conclusions are not artefacts of a single chip we re-ran
the full 2×2 on two additional Android devices --- a Motorola Moto G30 (Qualcomm
Snapdragon 662, Cortex-A73 cluster, ARMv8.0 baseline, 4 GB LPDDR4) and a Huawei
P20 Lite ANE-LX3 (HiSilicon Kirin 659, Cortex-A53, ARMv8.0 baseline, 4 GB
LPDDR4) --- using exactly the same model files, the same 2 000-passage NQ slice
(sha256 prefix \texttt{63be4b8894c71ff3}), and the same Dazzle build
(\texttt{libdazzle.so} baseline, no \texttt{\_v82} variant on v8.0 cores). On
the two HNSW-capable chips all four cells reproduce with overlapping 95 \% CIs
on \texttt{F1\_short} and identical paired-ratio significance:
...
paired-qid resampling. The Kirin 659 row uses \texttt{Algorithm.FLAT} instead of
\texttt{HNSW} (see sidebar item 6); FLAT recall at this scale (N = 2 000, dim =
384, k = 5) is \textgreater{} 99 \% so per-cell \texttt{F1\_short} differs by at
most \textasciitilde0.005 between the two algorithms --- within the per-cell
CI.}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0723}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0843}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0602}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0602}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.0723}}
  >{\raggedright\arraybackslash}p{(\linewidth - 16\tabcolsep) * \real{0.1687}}
...
\bottomrule\noalign{}
\endlastfoot
Unisoc T760 (Moto G35 5G) & A76 & v8.2 & 6 GB & HNSW & 0.079 {[}0.055, 0.105{]}
& 0.235 {[}0.191, 0.283{]} & 0.118 {[}0.084, 0.154{]} & 0.487 {[}0.431,
0.542{]} \\
QCOM SD662 (Moto G30) & A73 & v8.0 & 4 GB & HNSW & 0.079 {[}0.055, 0.105{]} &
0.240 {[}0.195, 0.287{]} & 0.118 {[}0.084, 0.154{]} & 0.487 {[}0.431,
0.542{]} \\
HiSi Kirin 659 (Huawei P20 Lite) & A53 & v8.0 & 4 GB & FLAT & 0.079 {[}0.055,
0.105{]} & 0.236 {[}0.191, 0.283{]} & 0.119 {[}0.085, 0.155{]} & 0.484 {[}0.427,
0.539{]} \\
\end{longtable}
}

All three chips reproduce the same four cells at the per-cell CI. Three of the
four cells (\texttt{small\ no-RAG}, \texttt{large\ no-RAG},
...
three SoCs at the same significance level. The reproduction also confirms that
\textbf{the §5.9 conclusions are storage-engine claims, not LLM-runtime claims}:
the chips differ by 3× in raw LLM throughput (T760's \texttt{large\ +\ RAG} p50
is 49.23 s, SD662's 71.99 s, Kirin 659's 152.26 s) but the F1 ratios that drive
the §5.9 thesis are stable. The Kirin row also confirms the storage-engine
thesis from the opposite direction: retrieval (FLAT scalar scan @
\textasciitilde2.5 ms p50 for N = 2 000, dim = 384) and ingest (addBatchDirect
47 ms for N = 2 000) operate at SDK-default budgets even on a 2017 Cortex-A53 /
4 GB chip, so the engine surface documented in this paper is portable across all
three Android microarchitecture generations tested (A76, A73, A53).
...
Prefill dominates, retrieval is \textless{} 0.01 \% of the total even on a 2017
Cortex-A53 chip, the prefill ≈ 6× decode ratio that Qwen 2.5's compute-vs-memory
profile sets is invariant across microarchitectures.}
...
\item
  ...
  the microarchitecture. T760 is 3.2× faster than Kirin in absolute terms, but
  the intra-query split is the same.
\item
...
Items 1--5 are universal portability fixes that landed across all three chips.
Items 6--10 are specific to the Kirin 659 path and do not change the numerical
comparison on the two HNSW chips in Table 17. They close the gap between ``the
bench ran on the chip we developed it on'' and ``the bench runs on a spread of
mid-range Android SoCs'' --- a precondition for any cross-platform claim.
\end{quote}
