...
(including Llama 3.2 1B and 3B), MobileLLM \citep{mobilellm}, and the Qwen 2.5
series \citep{qwen25} we use in §5.9. The runtime stack --- \texttt{llama.cpp}
...
the dominant embedded options --- SQLite \citep{sqlite}, RocksDB \citep{rocksdb}
(an LSM-tree descendant of LevelDB \citep{leveldb}), LMDB \citep{lmdb},
ObjectBox \citep{objectbox4}, and the analytical embedded peer DuckDB
\citep{duckdb} --- expose between two and four data structures total: rows,
key-value pairs, blob stores, occasionally a vector index. Every higher-order
construct (bounded streams, materialized aggregates, approximate counts, TTL
semantics, similarity search) has to be reimplemented in application code on top
of those primitives, by every team that needs them. The resulting code is
fragile under concurrent access. The atomicity guarantees that come for free
with a mature in-process server are silently lost. The same handful of bugs gets
rediscovered in app after app.
...
\citep{lewisrag, karpukhindpr} --- that are not achievable when the only
...
Moto G35 5G) is roughly 0.0001 \% of a single Gemma 4 E2B \citep{gemma4}
...
with native HNSW support --- both are HNSW \citep{hnsw} with expected
\(O(\log N)\) search.
...
\citep{litertlm, llamacpp, mlcllm, executorch} target model compression and
...
\citep{sqliteaivector} (\texttt{github.com/sqliteai/sqlite-vector}, Elastic
...
and i8 cosine come from \texttt{simsimd} \citep{simsimd} (Vardanian, Apache 2.0
