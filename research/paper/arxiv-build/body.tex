...
\texttt{HNSW} (see sidebar) and shows a separate prompt-injection anomaly noted
below.}
...
0.105{]} & 0.025 {[}0.016, 0.037{]} & 0.119 {[}0.085, 0.155{]} & 0.099 {[}0.070,
0.131{]} \\
\end{longtable}
}

On the two HNSW chips three of the four cells are bit-identical (Q4\_K\_M
weights + greedy decoding + identical retrieved passages → identical token
streams), and the fourth --- \texttt{small\ +\ RAG} --- differs only by 0.005 in
the point estimate (0.235 vs 0.240) which is well within the per-cell CI.

The Kirin 659 row reports the run as it actually completed on the chip after a
\textbf{twelve-pass investigation} (FLAT-algorithm fallback for the HNSW
\texttt{addPoint(label\ =\ 0)} deadlock + a four-phase multi-process driver to
bypass EMUI 9 iAware kill-score accumulation --- see
\texttt{research/results/cross\_platform\_e2e/\ ane\_lx3\_kirin659\_investigation.md}).
The two \texttt{no-RAG} cells reproduce the other chips at the per-cell CI, but
the two \texttt{+RAG} cells lift \emph{less} than expected:
\texttt{f1\_vs\_gold\_passage} rises from 0.151 (no-RAG) to 0.234 (+RAG) on
Kirin, vs.~0.151 → 0.334 on T760. The \texttt{prompt\_tokens.avg} for the Kirin
\texttt{+RAG} variants is 37 vs.~570 on the other chips, indicating that
retrieved passages reach the LLM but with a Cortex-A53 / FLAT-path-specific
truncation of the top-k context. The retrieval pipeline operates (FLAT search
returns hits, lookups resolve, the F1 lift is positive) but the amount of
context delivered is short. The ratios for the two \texttt{+RAG} columns
therefore reflect \textbf{one chip with a partially injected context}, not a
contradiction of the §5.9 storage-engine thesis; the SDK v3 follow-up will
resolve the prompt-construction delta on the FLAT path so the Kirin row can be
re-baselined alongside the HNSW chips.
...
{[}3.10, 5.86{]} ★ \\
HiSi Kirin 659 & 0.21× {[}0.14, 0.33{]} ★ & 0.31× {[}0.22, 0.44{]} ★ & 0.84×
{[}0.63, 1.09{]} \\
\end{longtable}
}

The same three statements about retrieval-as-dominant-lever (§5.9.4) hold on
both HNSW SoCs at the same significance level. The reproduction also confirms
that \textbf{the §5.9 conclusions are storage-engine claims, not LLM-runtime
claims}: the chips differ by \textasciitilde1.5× in raw LLM throughput (T760's
\texttt{large\ +\ RAG} p50 is 49.23 s vs SD662's 71.99 s) but the F1 ratios that
drive the §5.9 thesis are stable. The Kirin 659 row, while subject to the
prompt-injection anomaly noted above, also confirms the storage-engine thesis
from the opposite direction: retrieval (FLAT @ 60 µs avg) and ingest
...
\item
  ...
  harness now ships a four-phase multi-process driver
  (\texttt{RagE2EBenchPhases}) that splits the bench across separate
  \texttt{am\ instrument} invocations: \texttt{phase=embed} writes embeddings to
  a binary cache, \texttt{phase=small} and \texttt{phase=large} open one LLM
  each in a fresh process, \texttt{phase=merge} assembles the canonical
  \texttt{rag\_e2e\_*.json}. Total wall clock on Kirin 659: 25 min + 73 min +
  129 min + \textless{} 1 s = ≈ 3 hr 50 min.
\end{enumerate}

Items 1--5 are universal portability fixes that landed across all three chips.
Items 6 and 7 are specific to the Kirin 659 path and do not change the numerical
