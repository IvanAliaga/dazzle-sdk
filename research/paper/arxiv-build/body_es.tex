...
interactivas en hardware ARM64 de clase smartphone: Gemma 2 \citep{gemma2} y
Gemma 4 \citep{gemma4}, Phi-3 mini \citep{phi3}, la serie Llama 3 \citep{llama3}
(incluyendo Llama 3.2 1B y 3B), MobileLLM \citep{mobilellm}, y la serie Qwen 2.5
\citep{qwen25} que usamos en §5.9. El stack de runtime --- \texttt{llama.cpp}
\citep{llamacpp} con su formato de pesos cuantizados GGUF \citep{gguf},
LiteRT-LM \citep{litertlm}, MLC-LLM \citep{mlcllm}, ExecuTorch
\citep{executorch} --- está maduro y alcanza latencia sub-segundo por token
decodificado en handsets de \$150.
...
iOS las opciones embebidas dominantes --- SQLite \citep{sqlite}, RocksDB
\citep{rocksdb} (un descendiente LSM-tree de LevelDB \citep{leveldb}), LMDB
\citep{lmdb}, ObjectBox \citep{objectbox4}, y el peer embebido analítico DuckDB
\citep{duckdb} --- exponen entre dos y cuatro estructuras de datos en total:
...
\citep{lewisrag, karpukhindpr} --- que no son alcanzables cuando la única
primitiva disponible es un store key-value plano o de filas.
...
\item
  ...
  \(\sim 0.8\) \% error relativo \citep{hyperloglog} vs O(N) exacto).
\item
...
Pro vía el preset \texttt{paper384\_scale}. El RAG end-to-end de §5.9 corre
sobre el Moto G35 5G; la validación cross-platform end-to-end queda como trabajo
futuro en §6.3.
...
\citep{efronbootstrap, davisonhinkley} sobre las arrays per-query para los CIs
reportados en las tablas. Los scripts de re-derivación están en
...
Gemma 4 E2B \citep{gemma4} (1--3 segundos). La diferencia perceptible para el
...
mobile-first con soporte HNSW nativo: ambos son HNSW \citep{hnsw} con búsqueda
esperada \(O(\log N)\).
...
\citep{efronbootstrap, davisonhinkley} computado sobre los cuatro runs
...
\begin{quote}
\textbf{Alcance de esta sección.} La ablación RAG end-to-end reportada abajo
corre sobre un solo dispositivo físico (Motorola Moto G35 5G, handset Android
budget Snapdragon-class). La validación end-to-end cross-platform a través de
SoCs adicionales (y un run end-to-end iPhone 12 Pro, actualmente solo
storage-path) se reporta en la próxima revisión.
\end{quote}
...
distinto --- small-model + RAG on-device \citep{lewisrag, karpukhindpr} --- que
...
degradación de contexto-largo \citep{liulostmiddle} no están en el path entre el
...
\citep{nq2019} en un Motorola Moto G35 5G (Unisoc T760, 4 GB RAM, Android 14).
Los passages y los pares (question, gold-passage) vienen del split SBERT pair de
NQ \citep{sbertnq}; los aliases de short-answer canónicos se joinean desde
\texttt{nq\_open} \citep{nqopen, leeorqa} por texto de pregunta lowercased.
...
Embeddings producidos con \texttt{bge-small-en-v1.5} \citep{bgesmall} (q4\_k\_m,
...
\texttt{y\_gold} se normalizan siguiendo el protocolo SQuAD v1.1 \citep{squad}
...
² En Helio G80 (Cortex-A75 \citep{armcortexa75} big core) el binario v82
...
complexity \citep{hnsw}; SQLiteAI v0.9.95 implementa linear scan acelerado por
...
\citep{litertlm, llamacpp, mlcllm, executorch} apuntan a compresión de modelo y
eficiencia de runtime. Ninguno aborda la capa de ejecución stateful; cada
inference call se trata como un evento independiente.

\textbf{Memoria para agentes.} MemGPT \citep{memgpt} propone una jerarquía de
memoria de dos niveles (in-context + external) targeting server deployment.
Generative Agents \citep{generativeagents} usa un memory stream
retrieval-augmented para simulación NPC. A-MEM \citep{amem} introduce memoria
...
deliberadamente evita HNSW. \textbf{ObjectBox 4.x} \citep{objectbox4} es el otro
peer comercial en nuestro benchmark y ship HNSW nativo respaldado por vectores
float32.

\textbf{Librerías ANN vectoriales server-grade.} \textbf{FAISS} \citep{faiss}
(Facebook AI), \textbf{DiskANN} \citep{diskann} (Microsoft Research), y
\textbf{Milvus} \citep{milvus} (Zilliz) fijan el baseline open-source para
...
Product Quantisation \citep{jegoupq} en particular --- es el precedente
algorítmico para el path SQ8 que Dazzle usa.

Los kernels NEON de Dazzle para fp32 dot product, fp32 L2-squared, fp16 dot
product, e i8 cosine vienen de \texttt{simsimd} \citep{simsimd} (Vardanian,
...
\textbf{Object stores móviles más allá de SQLite.} \textbf{Realm} \citep{realm}
(ahora MongoDB) y \textbf{WCDB} \citep{wcdb} (Tencent / WeChat) son los dos
peers production-deployed de ObjectBox en el espacio móvil object-store. Ambos
...
\textbf{Evaluación de RAG.} \textbf{RAGTruth} \citep{ragtruth} es el corpus
