...
\subsubsection{5.9.5 Extensión cross-platform a tres SoCs
Android}\label{extensiuxf3n-cross-platform-a-tres-socs-android}

Para chequear que las conclusiones de §5.9 no son artefactos de un solo chip,
re-corrimos la 2×2 completa sobre dos dispositivos Android adicionales --- un
Motorola Moto G30 (Qualcomm Snapdragon 662, cluster Cortex-A73, baseline
ARMv8.0, 4 GB LPDDR4) y un Huawei P20 Lite ANE-LX3 (HiSilicon Kirin 659,
Cortex-A53, baseline ARMv8.0, 4 GB LPDDR4) --- usando exactamente los mismos
archivos de modelo, el mismo slice NQ de 2 000 passages (prefijo sha256
\texttt{63be4b8894c71ff3}), y el mismo build de Dazzle (\texttt{libdazzle.so}
baseline, sin variante \texttt{\_v82} en cores v8.0). Sobre los dos chips HNSW
las cuatro celdas reproducen con CIs 95 \% superpuestos sobre \texttt{F1\_short}
y misma significancia paired-ratio:
...
paired-qid resampling. La fila del Kirin 659 corre con \texttt{Algorithm.FLAT}
en lugar de \texttt{HNSW} (ver sidebar) y muestra una anomalía separada de
prompt-injection notada abajo.}
...
Sobre los dos chips HNSW tres de las cuatro celdas son bit-idénticas (pesos
Q4\_K\_M + greedy decoding + passages retrieved idénticos → token stream
idéntico), y la cuarta --- \texttt{small\ +\ RAG} --- solo difiere por 0.005 en
el punto estimado (0.235 vs 0.240), bien dentro del CI por celda.

La fila del Kirin 659 reporta el run tal como completó sobre el chip después de
una \textbf{investigación de doce passes} (fallback al algoritmo FLAT por el
deadlock HNSW \texttt{addPoint(label\ =\ 0)} + driver multi-process de cuatro
fases para evadir la acumulación del kill-score de iAware en EMUI 9 --- ver
\texttt{research/results/cross\_platform\_e2e/ane\_lx3\_kirin659\_investigation.md}).
Las dos celdas \texttt{no-RAG} reproducen los otros chips dentro del CI por
celda, pero las dos celdas \texttt{+RAG} levantan \emph{menos} de lo esperado:
\texttt{f1\_vs\_gold\_passage} sube de 0.151 (no-RAG) a 0.234 (+RAG) en Kirin,
vs 0.151 → 0.334 en T760. El \texttt{prompt\_tokens.avg} de las variantes
\texttt{+RAG} en Kirin es 37 vs 570 en los otros chips, indicando que los
passages retrieved llegan al LLM pero con un truncamiento Cortex-A53 /
FLAT-path-specific del top-k context. El pipeline de retrieval opera (la
búsqueda FLAT devuelve hits, los lookups resuelven, el lift en F1 es positivo)
pero la cantidad de contexto entregado es corta. Las ratios de las dos columnas
\texttt{+RAG} reflejan entonces \textbf{un chip con context parcialmente
inyectado}, no una contradicción de la tesis de storage-engine de §5.9; el
follow-up SDK v3 va a resolver el delta de prompt-construction en el path FLAT
para que la fila del Kirin pueda ser re-baselined al lado de los chips HNSW.
...
EN) se sostienen sobre los dos SoCs HNSW al mismo nivel de significancia. La
reproducción también confirma que \textbf{las conclusiones de §5.9 son claims
sobre el storage engine, no sobre el LLM runtime}: los chips difieren
\textasciitilde1.5× en throughput LLM bruto (p50 de \texttt{large\ +\ RAG} en
T760 es 49.23 s vs 71.99 s en SD662) pero las ratios de F1 que sostienen la
tesis de §5.9 son estables. La fila del Kirin 659, aunque sujeta a la anomalía
de prompt-injection notada arriba, también confirma la tesis de storage-engine
desde la dirección opuesta: retrieval (FLAT @ 60 µs avg) e ingest
(addBatchDirect 47 ms para N = 2 000) operan sobre los presupuestos default del
SDK incluso sobre un chip Cortex-A53 / 4 GB del 2017, así que la surface del
engine documentada en este paper es portable sobre las tres generaciones de
microarquitectura Android testeadas (A76, A73, A53).
...
\item
  ...
  límite OS-wide del hardware. El harness ahora ships un driver multi-process de
  ...
  a un cache binary, \texttt{phase=small} y \texttt{phase=large} abren un LLM
  cada uno en un proceso fresh, \texttt{phase=merge} arma el
  \texttt{rag\_e2e\_*.json} canónico. Wall clock total en Kirin 659: 25 min + 73
  min + 129 min + \textless{} 1 s = ≈ 3 hr 50 min.
\end{enumerate}

Los items 1--5 son fixes universales de portabilidad que landed sobre los tres
chips. Los items 6 y 7 son específicos del path Kirin 659 y no cambian la
comparación numérica sobre los dos chips HNSW de la Tabla 17. Cierran el gap
entre ``el bench corrió sobre el chip donde lo desarrollamos'' y ``el bench
corre sobre un spread de SoCs Android mid-range'' --- una precondición para
