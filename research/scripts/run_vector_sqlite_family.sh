...
#
# Runs the focused SQLite-family vector benchmark on Android:
#   backend = vector-bench-sqlite-family
#
# Output JSON pattern on device:
#   /sdcard/Documents/vecbench_sqlite_family_<MODEL>_<ts>.json
#
# Usage:
#   research/scripts/run_vector_sqlite_family.sh
#   research/scripts/run_vector_sqlite_family.sh --rounds 5 --cooldown 2
#   research/scripts/run_vector_sqlite_family.sh --results research/benchmarks/results/vector_sqlite_family
#
set -euo pipefail
...
TIMEOUT=900
COOLDOWN_SEC=2
BASE_RESULTS_DIR="$REPO_ROOT/research/benchmarks/results/vector_sqlite_family"
...
GLOB="/sdcard/Documents/vecbench_sqlite_family_*.json"
...
for RUN_IDX in $(seq 1 "$ROUNDS"); do
    ...
    adb -s "$DEVICE" shell am start -n "$PKG/.ExperimentActivity" \
        ...
        --es backend vector-bench-sqlite-family >/dev/null
...
log "analyze with:"
log "  python3 research/scripts/analyze_vector_sqlite_family.py $OUT_DIR"
